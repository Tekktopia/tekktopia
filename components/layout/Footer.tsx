...
export default function Footer() {
  ...
  return (
    <footer
      ...
      style={{ background: "#04080F", borderTop: "1px solid rgba(255,255,255,0.06)" }}
    >
      {/* Fine grid */}
      ...
      {/* Subtle top glow */}
      <div aria-hidden className="pointer-events-none absolute top-0 left-0 right-0"
        style={{ height: 280, background: "radial-gradient(ellipse 70% 100% at 50% 0%,rgba(37,99,235,0.07) 0%,transparent 100%)" }}
      />
      ...
      <div className="relative max-w-7xl mx-auto px-5 sm:px-8 md:px-12 pt-16 pb-14 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-10">
        ...
        <div className="lg:col-span-4 flex flex-col">
          ...
          <Link href="/" className="group inline-flex items-center gap-3 mb-7 w-fit">
            ...
            />
            <span className="font-display font-bold text-xl tracking-tight" style={{ color: "#fff" }}>
              tekk<span style={{ color: "#F97316" }}>topia</span>
            </span>
          </Link>

          {/* Tagline */}
          <p style={{ fontSize: 13.5, color: "rgba(255,255,255,0.55)", fontWeight: 300, lineHeight: 1.78, maxWidth: 300, marginBottom: 28 }}>
            We build the technology that drives your business forward — from managed IT to custom software, cloud, and beyond.
          </p>

          {/* Contact */}
          <div className="flex flex-col gap-3 mb-8">
            <a href="mailto:hello@tekktopialtd.com"
              className="inline-flex items-center gap-2.5 w-fit group"
              style={{ fontSize: 13, fontWeight: 500, color: "#F97316", transition: "opacity 0.2s" }}
              onMouseEnter={e => (e.currentTarget.style.opacity = "0.75")}
              onMouseLeave={e => (e.currentTarget.style.opacity = "1")}
            >
              <Mail style={{ width: 14, height: 14, flexShrink: 0 }} strokeWidth={1.8} />
              hello@tekktopialtd.com
            </a>
            <a href="tel:+44000000000"
              className="inline-flex items-center gap-2.5 w-fit"
              style={{ fontSize: 13, fontWeight: 400, color: "rgba(255,255,255,0.45)", transition: "color 0.2s" }}
              onMouseEnter={e => (e.currentTarget.style.color = "rgba(255,255,255,0.85)")}
              onMouseLeave={e => (e.currentTarget.style.color = "rgba(255,255,255,0.45)")}
            >
              <Phone style={{ width: 14, height: 14, flexShrink: 0 }} strokeWidth={1.8} />
              +44 (0) 000 000 0000
            </a>
          </div>
        ...
        <div className="lg:col-span-3">
          <h3 style={{ fontFamily: "monospace", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.28em", color: "rgba(255,255,255,0.35)", marginBottom: 20 }}>
            Services
          </h3>
          <ul className="flex flex-col gap-3">
            {SERVICE_LINKS.map(({ label, href }) => (
              <li key={href}>
                <Link href={href}
                  style={{ fontSize: 13, color: "rgba(255,255,255,0.52)", transition: "color 0.2s" }}
        ...
        <div className="lg:col-span-3">
          ...
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.52)", lineHeight: 1.72, marginBottom: 18 }}>
          ...
          <form className="flex flex-col gap-3" onSubmit={e => e.preventDefault()}>
            ...
            <button
              ...
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.boxShadow = "0 0 32px rgba(249,115,22,0.45)"; (e.currentTarget as HTMLElement).style.transform = "translateY(-1px)"; }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.boxShadow = "none"; (e.currentTarget as HTMLElement).style.transform = "none"; }}
            >
              Subscribe
              <ArrowUpRight style={{ width: 14, height: 14 }} className="transition-transform duration-300 group-hover:rotate-45" />
            </button>
          </form>
        </div>
      ...
      <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", position: "relative", overflow: "hidden" }}>
        ...
        <div aria-hidden className="pointer-events-none absolute inset-0 flex items-center justify-center select-none"
          ...
          <span className="font-display font-black uppercase"
            style={{ fontSize: "clamp(60px,12vw,160px)", color: "rgba(255,255,255,0.022)", letterSpacing: "-0.04em", whiteSpace: "nowrap", lineHeight: 1 }}>
            TEKKTOPIA
          </span>
        </div>
        ...
        <div className="relative max-w-7xl mx-auto px-5 sm:px-8 md:px-12 py-7 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex flex-col gap-1">
            <p style={{ fontSize: 11, color: "rgba(255,255,255,0.32)", fontFamily: "monospace" }}>
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-2">
            {LEGAL_LINKS.map(({ label, href }) => (
              <Link key={href} href={href}
                style={{ fontSize: 11, fontFamily: "monospace", color: "rgba(255,255,255,0.32)", transition: "color 0.2s" }}
